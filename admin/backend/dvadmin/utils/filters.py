...
class DataLevelPermissionsSubFilter(DataLevelPermissionsFilter):
    ...
    def _extracted_from_filter_queryset_33(self, request:Request, queryset, api, method):
        ...
        if u.is_superuser:
            return queryset
        # If the model doesn't define data-scope fields, skip data-level filtering
        model_field_names = {f.name for f in queryset.model._meta.fields}
        if 'dept_belong_id' not in model_field_names and 'creator' not in model_field_names:
            return queryset
        # (0, "仅本人数据权限"),
        # (1, "本部门及以下数据权限"),
        # (2, "本部门数据权限"),
        ...
        if queryset.model._meta.model_name == 'dept':
            if 'creator' in model_field_names:
                return queryset.filter(Q(id__in=dept_list) | Q(creator=u))
            return queryset.filter(Q(id__in=dept_list))
        if 'dept_belong_id' in model_field_names and 'creator' in model_field_names:
            return queryset.filter(Q(dept_belong_id__in=dept_list) | Q(creator=u))
        if 'dept_belong_id' in model_field_names:
            return queryset.filter(Q(dept_belong_id__in=dept_list))
        if 'creator' in model_field_names:
            return queryset.filter(Q(creator=u))
        return queryset
